  ...
  if (job?.category) {
    const fetchRelated = async () => {
      try {
        ...
        // Filter out the current job so it doesn't suggest itself
    ...
    <div className="bg-base-200 min-h-screen pt-28 pb-20">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* LEFT: Media & Overview (8 Cols) */}
          <div className="lg:col-span-8 space-y-8">
            
            {/* Media Gallery Section */}
            {/* <div className="bg-base-100 rounded-[2rem] overflow-hidden shadow-sm border border-base-300">
              <div className="relative group">
                <img src={job.coverImage} alt={job.title} className="w-full h-[450px] object-cover transition-transform duration-700 group-hover:scale-105" />
                <div className="absolute top-4 left-4">
                   <span className="badge badge-primary p-4 font-bold shadow-lg uppercase">{job.category}</span>
                </div>
              </div>
              {/* Media Thumbnails - Requirement: Multiple Images */}
              {/* <div className="p-4 flex gap-4 overflow-x-auto bg-base-100">
                <img src={job.coverImage} className="w-24 h-20 rounded-2xl object-cover border-4 border-primary shadow-md" />
                {[1, 2].map((i) => (
                   <div key={i} className="w-24 h-20 rounded-2xl bg-base-200 flex items-center justify-center text-xs font-bold opacity-40 border border-dashed border-base-content/20">
                      View <br/> Media {i+1}
                   </div>
                ))}
...
<div className="space-y-4">
  ...
  {/* Thumbnails (Requirement #4: Multiple Media) */}
    ...
    {job.extraImages && job.extraImages.map((imgUrl, index) => (
      imgUrl && (
        ...
        />
          ...
          {/* RIGHT: Key Information & Sidebar (4 Cols) */}
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-base-100 p-8 rounded-[2rem] shadow-sm border border-base-300 sticky top-28">
              ...
              {/* Suggested/Safety Section - Requirement: Related/Suggested */}
        ...
        {/* Related/Suggested Items Section (Requirement #4) */}
